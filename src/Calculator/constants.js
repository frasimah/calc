export const CONFIG = {
    SERVICES: {
        1: "Target_pricing",
        2: "Retargeting pricing",
        3: "Колл-центр"
    },
    SERVICE_PRICING: {
        1: [
            { contacts: 0, price_per_contact: 70 },
            { contacts: 1000, price_per_contact: 60 },
            { contacts: 3000, price_per_contact: 50 },
            { contacts: 4000, price_per_contact: 45 },
            { contacts: 5000, price_per_contact: 43 },
            { contacts: 6000, price_per_contact: 40 },
            { contacts: 7000, price_per_contact: 36 },
            { contacts: 8000, price_per_contact: 34 },
            { contacts: 9000, price_per_contact: 32 },
            { contacts: 10000, price_per_contact: 30 },
            { contacts: 13000, price_per_contact: 29 },
            { contacts: 15000, price_per_contact: 28 },
            { contacts: 20000, price_per_contact: 26 },
            { contacts: 25000, price_per_contact: 25 },
            { contacts: 30000, price_per_contact: 24 },
            { contacts: 35000, price_per_contact: 23 },
            { contacts: 40000, price_per_contact: 22 },
            { contacts: 45000, price_per_contact: 21 },
            { contacts: 50000, price_per_contact: 20 },
            { contacts: 60000, price_per_contact: 19 },
            { contacts: 70000, price_per_contact: 18 },
            { contacts: 80000, price_per_contact: 17 },
            { contacts: 90000, price_per_contact: 16 },
            { contacts: 100000, price_per_contact: 15 }
        ],
        2: [
            { contacts: 0, price_per_contact: 70 },
            { contacts: 1000, price_per_contact: 60 },
            { contacts: 3000, price_per_contact: 50 },
            { contacts: 4000, price_per_contact: 45 },
            { contacts: 5000, price_per_contact: 43 },
            { contacts: 6000, price_per_contact: 40 },
            { contacts: 7000, price_per_contact: 36 },
            { contacts: 8000, price_per_contact: 34 },
            { contacts: 9000, price_per_contact: 32 },
            { contacts: 10000, price_per_contact: 30 },
            { contacts: 13000, price_per_contact: 29 },
            { contacts: 15000, price_per_contact: 28 },
            { contacts: 20000, price_per_contact: 26 },
            { contacts: 25000, price_per_contact: 25 },
            { contacts: 30000, price_per_contact: 24 },
            { contacts: 35000, price_per_contact: 23 },
            { contacts: 40000, price_per_contact: 22 },
            { contacts: 45000, price_per_contact: 21 },
            { contacts: 50000, price_per_contact: 20 },
            { contacts: 60000, price_per_contact: 19 },
            { contacts: 70000, price_per_contact: 18 },
            { contacts: 80000, price_per_contact: 17 },
            { contacts: 90000, price_per_contact: 16 },
            { contacts: 100000, price_per_contact: 15 }
        ],
        3: [
            { contacts: 0, price_per_call: 50 },
            { contacts: 1000, price_per_call: 45 },
            { contacts: 3000, price_per_call: 40 },
            { contacts: 4000, price_per_call: 35 },
            { contacts: 5000, price_per_call: 34 },
            { contacts: 6000, price_per_call: 33 },
            { contacts: 7000, price_per_call: 32 },
            { contacts: 8000, price_per_call: 31 },
            { contacts: 9000, price_per_call: 30 },
            { contacts: 10000, price_per_call: 29 },
            { contacts: 13000, price_per_call: 28 },
            { contacts: 15000, price_per_call: 27 },
            { contacts: 20000, price_per_call: 26 },
            { contacts: 25000, price_per_call: 25 },
            { contacts: 30000, price_per_call: 24 },
            { contacts: 35000, price_per_call: 23 },
            { contacts: 40000, price_per_call: 22 },
            { contacts: 45000, price_per_call: 21 },
            { contacts: 50000, price_per_call: 20 }
        ]
    },
    TECH_PACKAGES: {
        "Tech": { "price": 4990, "discount": 0.0 },
        "Must": { "price": 9990, "discount": 0.05 },
        "Pro": { "price": 14990, "discount": 0.1 },
        "Call": { "price": 4990, "discount": 0.0 }
    },
    SERVICE_OPTIONS: [
        { id: 1, name: "Segment Scoring" },
        { id: 2, name: "Retargeting Trigger Leads" },
        { id: 3, name: "Reactivation/Validation" }
    ],
    SUBSCRIPTION_OPTIONS: [
        { id: "tech", name: "Tech" },
        { id: "must", name: "Must" },
        { id: "pro", name: "Pro" },
        { id: "call", name: "Call" }
    ],
    SLIDER: {
        MIN: 1000,
        MAX: 100000,
        STEP: 100
    },
    CONTACT_PROCESSING_PRICING: [
        { contacts: 0, price_per_contact: 50.0 },
        { contacts: 1000, price_per_contact: 45.0 },
        { contacts: 3000, price_per_contact: 38.5 },
        { contacts: 4000, price_per_contact: 30.0 },
        { contacts: 5000, price_per_contact: 26.5 },
        { contacts: 6000, price_per_contact: 26.0 },
        { contacts: 7000, price_per_contact: 25.5 },
        { contacts: 8000, price_per_contact: 25.0 },
        { contacts: 9000, price_per_contact: 24.5 },
        { contacts: 10000, price_per_contact: 24.0 },
        { contacts: 13000, price_per_contact: 22.5 },
        { contacts: 15000, price_per_contact: 21.5 },
        { contacts: 20000, price_per_contact: 20.5 },
        { contacts: 25000, price_per_contact: 20.0 },
        { contacts: 30000, price_per_contact: 19.5 },
        { contacts: 35000, price_per_contact: 19.0 },
        { contacts: 40000, price_per_contact: 18.5 },
        { contacts: 45000, price_per_contact: 18.0 },
        { contacts: 50000, price_per_contact: 17.5 },
        { contacts: 60000, price_per_contact: 17.0 },
        { contacts: 70000, price_per_contact: 16.5 },
        { contacts: 80000, price_per_contact: 16.0 },
        { contacts: 90000, price_per_contact: 15.5 },
        { contacts: 100000, price_per_contact: 15.0 }
    ],
    // Пакетные прайс-таблицы стоимости данных для Must/Pro (по документу)
    SERVICE_PRICING_BY_PACKAGE: {
        Must: {
            1: [
                { contacts: 0, price_per_contact: 50.0 },
                { contacts: 1000, price_per_contact: 45.0 },
                { contacts: 3000, price_per_contact: 38.5 },
                { contacts: 4000, price_per_contact: 30.0 },
                { contacts: 5000, price_per_contact: 26.5 },
                { contacts: 6000, price_per_contact: 26.0 },
                { contacts: 7000, price_per_contact: 25.5 },
                { contacts: 8000, price_per_contact: 25.0 },
                { contacts: 9000, price_per_contact: 24.5 },
                { contacts: 10000, price_per_contact: 24.0 },
                { contacts: 13000, price_per_contact: 22.5 },
                { contacts: 15000, price_per_contact: 21.5 },
                { contacts: 20000, price_per_contact: 20.5 },
                { contacts: 25000, price_per_contact: 20.0 },
                { contacts: 30000, price_per_contact: 19.5 },
                { contacts: 35000, price_per_contact: 19.0 },
                { contacts: 40000, price_per_contact: 18.5 },
                { contacts: 45000, price_per_contact: 18.0 },
                { contacts: 50000, price_per_contact: 17.5 },
                { contacts: 60000, price_per_contact: 17.0 },
                { contacts: 70000, price_per_contact: 16.5 },
                { contacts: 80000, price_per_contact: 16.0 },
                { contacts: 90000, price_per_contact: 15.5 },
                { contacts: 100000, price_per_contact: 15.0 }
            ],
            2: [
                { contacts: 0, price_per_contact: 50.0 },
                { contacts: 1000, price_per_contact: 45.0 },
                { contacts: 3000, price_per_contact: 38.5 },
                { contacts: 4000, price_per_contact: 30.0 },
                { contacts: 5000, price_per_contact: 26.5 },
                { contacts: 6000, price_per_contact: 26.0 },
                { contacts: 7000, price_per_contact: 25.5 },
                { contacts: 8000, price_per_contact: 25.0 },
                { contacts: 9000, price_per_contact: 24.5 },
                { contacts: 10000, price_per_contact: 24.0 },
                { contacts: 13000, price_per_contact: 22.5 },
                { contacts: 15000, price_per_contact: 21.5 },
                { contacts: 20000, price_per_contact: 20.5 },
                { contacts: 25000, price_per_contact: 20.0 },
                { contacts: 30000, price_per_contact: 19.5 },
                { contacts: 35000, price_per_contact: 19.0 },
                { contacts: 40000, price_per_contact: 18.5 },
                { contacts: 45000, price_per_contact: 18.0 },
                { contacts: 50000, price_per_contact: 17.5 },
                { contacts: 60000, price_per_contact: 17.0 },
                { contacts: 70000, price_per_contact: 16.5 },
                { contacts: 80000, price_per_contact: 16.0 },
                { contacts: 90000, price_per_contact: 15.5 },
                { contacts: 100000, price_per_contact: 15.0 }
            ]
        },
        Pro: {
            1: [
                { contacts: 0, price_per_contact: 50.0 },
                { contacts: 1000, price_per_contact: 45.0 },
                { contacts: 3000, price_per_contact: 38.5 },
                { contacts: 4000, price_per_contact: 30.0 },
                { contacts: 5000, price_per_contact: 26.5 },
                { contacts: 6000, price_per_contact: 26.0 },
                { contacts: 7000, price_per_contact: 25.5 },
                { contacts: 8000, price_per_contact: 25.0 },
                { contacts: 9000, price_per_contact: 24.5 },
                { contacts: 10000, price_per_contact: 24.0 },
                { contacts: 13000, price_per_contact: 22.5 },
                { contacts: 15000, price_per_contact: 21.5 },
                { contacts: 20000, price_per_contact: 20.5 },
                { contacts: 25000, price_per_contact: 20.0 },
                { contacts: 30000, price_per_contact: 19.5 },
                { contacts: 35000, price_per_contact: 19.0 },
                { contacts: 40000, price_per_contact: 18.5 },
                { contacts: 45000, price_per_contact: 18.0 },
                { contacts: 50000, price_per_contact: 17.5 },
                { contacts: 60000, price_per_contact: 17.0 },
                { contacts: 70000, price_per_contact: 16.5 },
                { contacts: 80000, price_per_contact: 16.0 },
                { contacts: 90000, price_per_contact: 15.5 },
                { contacts: 100000, price_per_contact: 15.0 }
            ],
            2: [
                { contacts: 0, price_per_contact: 50.0 },
                { contacts: 1000, price_per_contact: 45.0 },
                { contacts: 3000, price_per_contact: 38.5 },
                { contacts: 4000, price_per_contact: 30.0 },
                { contacts: 5000, price_per_contact: 26.5 },
                { contacts: 6000, price_per_contact: 26.0 },
                { contacts: 7000, price_per_contact: 25.5 },
                { contacts: 8000, price_per_contact: 25.0 },
                { contacts: 9000, price_per_contact: 24.5 },
                { contacts: 10000, price_per_contact: 24.0 },
                { contacts: 13000, price_per_contact: 22.5 },
                { contacts: 15000, price_per_contact: 21.5 },
                { contacts: 20000, price_per_contact: 20.5 },
                { contacts: 25000, price_per_contact: 20.0 },
                { contacts: 30000, price_per_contact: 19.5 },
                { contacts: 35000, price_per_contact: 19.0 },
                { contacts: 40000, price_per_contact: 18.5 },
                { contacts: 45000, price_per_contact: 18.0 },
                { contacts: 50000, price_per_contact: 17.5 },
                { contacts: 60000, price_per_contact: 17.0 },
                { contacts: 70000, price_per_contact: 16.5 },
                { contacts: 80000, price_per_contact: 16.0 },
                { contacts: 90000, price_per_contact: 15.5 },
                { contacts: 100000, price_per_contact: 15.0 }
            ]
        }
    },
    CONVERSION: {
        BASE_BY_SERVICE: {
            1: 0.01, // Segment Scoring
            2: 0.02, // Retargeting Trigger Leads
            3: 0.03  // Reactivation/Validation
        },
        MULTIPLIERS: [
            { min: 0, max: 100000, multiplier: 2 },
            { min: 100000, max: 1000000, multiplier: 1.5 },
            { min: 1000000, max: 3000000, multiplier: 1 },
            { min: 3000000, max: Infinity, multiplier: 0.5 }
        ]
    }
};